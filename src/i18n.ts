import i18n from 'i18next'
import { initReactI18next } from 'react-i18next'

const resources = {
  pt: {
    translation: {
      app: {
        name: 'RPG Turbo',
        loading: 'Carregando sessao...',
      },
      lang: {
        pt: 'PT',
        en: 'EN',
      },
      user: {
        profile: 'Perfil',
        menuLabel: 'Menu do usuario',
        initialFallback: 'U',
      },
      navigation: {
        back: 'Voltar',
        breadcrumbs: 'Caminho de navegacao',
      },
      auth: {
        title: 'Entrar na sua mesa',
        subtitle: 'Use email e senha ou continue com Google.',
        tabSignIn: 'Entrar',
        tabSignUp: 'Cadastrar',
        nicknameLabel: 'Nickname',
        nicknamePlaceholder: 'Escolha um nickname unico',
        emailLabel: 'Email',
        emailPlaceholder: 'voce@exemplo.com',
        passwordLabel: 'Senha',
        passwordPlaceholder: 'Digite sua senha',
        signInWithEmail: 'Entrar com email',
        signInWithGoogle: 'Entrar com Google',
        signUpWithEmail: 'Cadastrar com email',
        signUpWithGoogle: 'Cadastrar com Google',
        signOut: 'Sair',
        invalidCredentials: 'Nao foi possivel entrar com email e senha.',
        googleSignInError: 'Nao foi possivel entrar com Google.',
        signUpError: 'Nao foi possivel cadastrar. Verifique email, senha e nickname unico.',
        signUpGoogleError: 'Nao foi possivel cadastrar com Google. Verifique nickname unico.',
      },
      campaigns: {
        title: 'Minhas campanhas',
        newCampaign: 'Nova campanha',
        createTitle: 'Criar campanha',
        editTitle: 'Editar campanha',
        viewTitle: 'Campanha',
        editCta: 'Editar campanha',
        backToList: 'Voltar para campanhas',
        ownerOnlyEditNotice: 'Somente o dono da campanha pode editar, salvar e convidar jogadores.',
        emptyDescription: 'Sem descricao cadastrada.',
        viewLoading: 'Carregando campanha...',
        viewNotFound: 'Campanha nao encontrada.',
        createdAt: 'Criada em',
        notifications: 'Notificacoes',
        noNotifications: 'Voce nao tem convites pendentes.',
        inviteMessage: 'Convite para a campanha {{campaignName}}',
        acceptInvite: 'Aceitar',
        declineInvite: 'Recusar',
        removeMember: 'Excluir usuario da campanha',
        invitesStatusTitle: 'Status dos convites',
        invitesEditorTitle: 'Convites por nickname',
        noInviteStatus: 'Nenhum convite enviado.',
        sendInvites: 'Convidar',
        removeSelectedNickname: 'Remover nickname selecionado',
        imagePlaceholder: 'Selecione uma imagem',
        imagePreviewAlt: 'Pre-visualizacao da capa da campanha',
        form: {
          name: 'Nome da campanha',
          description: 'Descricao',
          password: 'Senha (opcional)',
          image: 'Imagem da campanha (opcional)',
          imageHint: 'A imagem sera exibida aqui antes e depois de salvar a campanha.',
          inviteByNickname: 'Convidar jogador por nickname',
          save: 'Salvar campanha',
          create: 'Criar campanha',
        },
        status: {
          pending: 'Pendente',
          accepted: 'Aceito',
          declined: 'Recusado',
          revoked: 'Removido',
        },
      },
    },
  },
  en: {
    translation: {
      app: {
        name: 'RPG Turbo',
        loading: 'Loading session...',
      },
      lang: {
        pt: 'PT',
        en: 'EN',
      },
      user: {
        profile: 'Profile',
        menuLabel: 'User menu',
        initialFallback: 'U',
      },
      navigation: {
        back: 'Back',
        breadcrumbs: 'Breadcrumbs',
      },
      auth: {
        title: 'Sign in to your table',
        subtitle: 'Use email and password or continue with Google.',
        tabSignIn: 'Sign in',
        tabSignUp: 'Sign up',
        nicknameLabel: 'Nickname',
        nicknamePlaceholder: 'Choose a unique nickname',
        emailLabel: 'Email',
        emailPlaceholder: 'you@example.com',
        passwordLabel: 'Password',
        passwordPlaceholder: 'Enter your password',
        signInWithEmail: 'Sign in with email',
        signInWithGoogle: 'Sign in with Google',
        signUpWithEmail: 'Sign up with email',
        signUpWithGoogle: 'Sign up with Google',
        signOut: 'Sign out',
        invalidCredentials: 'Could not sign in with email and password.',
        googleSignInError: 'Could not sign in with Google.',
        signUpError: 'Could not sign up. Check email, password, and unique nickname.',
        signUpGoogleError: 'Could not sign up with Google. Check unique nickname.',
      },
      campaigns: {
        title: 'My campaigns',
        newCampaign: 'New campaign',
        createTitle: 'Create campaign',
        editTitle: 'Edit campaign',
        viewTitle: 'Campaign',
        editCta: 'Edit campaign',
        backToList: 'Back to campaigns',
        ownerOnlyEditNotice: 'Only the campaign owner can edit, save, and invite players.',
        emptyDescription: 'No description provided.',
        viewLoading: 'Loading campaign...',
        viewNotFound: 'Campaign not found.',
        createdAt: 'Created on',
        notifications: 'Notifications',
        noNotifications: 'You have no pending invites.',
        inviteMessage: 'Invite to campaign {{campaignName}}',
        acceptInvite: 'Accept',
        declineInvite: 'Decline',
        removeMember: 'Remove user from campaign',
        invitesStatusTitle: 'Invite status',
        invitesEditorTitle: 'Invite by nickname',
        noInviteStatus: 'No invites sent yet.',
        sendInvites: 'Invite',
        removeSelectedNickname: 'Remove selected nickname',
        imagePlaceholder: 'Select an image',
        imagePreviewAlt: 'Campaign cover preview',
        form: {
          name: 'Campaign name',
          description: 'Description',
          password: 'Password (optional)',
          image: 'Campaign image (optional)',
          imageHint: 'The image will be displayed here before and after saving the campaign.',
          inviteByNickname: 'Invite player by nickname',
          save: 'Save campaign',
          create: 'Create campaign',
        },
        status: {
          pending: 'Pending',
          accepted: 'Accepted',
          declined: 'Declined',
          revoked: 'Removed',
        },
      },
    },
  },
} as const

void i18n.use(initReactI18next).init({
  resources,
  lng: 'pt',
  fallbackLng: 'pt',
  interpolation: {
    escapeValue: false,
  },
})

export default i18n
